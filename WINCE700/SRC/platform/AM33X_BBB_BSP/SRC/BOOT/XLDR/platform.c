...
#define CONTROL_EMIF_SDRAM_CONFIG (CONTROL_BASE_ADDR + 0x110)
...
static void config_emif_ddr(EMIF_ddr *pemif_ddr)
{
    ...
    */
	...
	OUTREG32(OALPAtoUA(CONTROL_EMIF_SDRAM_CONFIG), DDR3_EMIF_SDCFG);
}
...
int board_init()
{
    ...
    if (g_dwBoardId == AM33X_BOARDID_BBONE_BOARD)
    {
		...
		TWLProtWriteRegs(PROT_LEVEL_NONE, PMIC_REG_POWER_PATH,
				       PMIC_POWER_PATH_IUSB_LIMIT_1300MA,
				       PMIC_POWER_PATH_IUSB_LIMIT_MASK);

        /* Set DCDC2 (MPU) voltage to 1.275V */
        TWLUpdateVoltage(PMIC_REG_DEFDCDC2,
					     DCDC_VOLT_SEL_1275MV);

        /* Set LDO3, LDO4 output voltage to 3.3V */
		TWLProtWriteRegs(PROT_LEVEL_2, PMIC_REG_DEFLS1,
