...
BOOL lcdc_cfg_dma(struct lcdc *lcdc)
{
	...
	lcdc->dma_fifo_thresh = LCDC_DMA_CTRL_TH_FIFO_READY_512;
       ...
       tmp = lcdc->regs->LCDDMA_CTRL & 0x00000001;
...
//	tmp |= (LCDC_RASTER_CTRL_RASTER_ORDER | LCDC_RASTER_CTRL_PALMODE(LCDC_LOAD_FRAME));
...
BOOL SetBacklightLevel_GPIO(UINT level)
{
    ...
    if(!g_hGpio)
    {
		...
		if (g_hGpio == NULL)
		{
			RETAILMSG(1, (L"SetBacklightLevel_GPIO:Failed to open GPIO driver\r\n"));
			return FALSE;
		}
    }

	RETAILMSG(1, ((L"SetBacklightLevel_GPIO: %d\r\n"),level));
...
LcdPdd_LCD_Initialize(struct lcdc *lcdc)
{
   ...
   //backlight is not available for beagle bone base board.
