...
DWORD MMCSetTranSpeed(DWORD speed)
{
    ...
    if (CurrentSpeed != speed)
	{
	    ...
	    OALMSG(OAL_INFO, (TEXT("SDCARD: reqested speed %d, actual speed %d\r\n"), speed, ActualSpeed));
        CurrentSpeed = speed;
    }		
    return ActualSpeed;
}
...
unsigned int MMCWaitForReady(PDISK pDisk)
{
         ...
         )
    {
        if (bTimeout)
        {
            OALMSGX(OAL_ERROR, (TEXT("SDMem: card rdy bTimeout, status = 0x%X\r\n"), MMCcmd.status));
            return 1;
        }
...
Detect4BitCardDone:
		...
		if (pDisk->d_Supports4Bit)
		{
			...
			{
				OALMSG(OAL_INFO, (TEXT("SDCARD:Using 4 bit mode\r\n")));
  ...
  //      if (MMCCommandResponse(&MMCcmd, 0))
  //      {
  //          OALMSGX(OAL_FUNC, (TEXT("MMC: unable to set 8 bitbus for MMC card.\r\n")));
  //          goto select_card_error;
  //      }
  //      else
  //      {
  ...
  //      }
		...
		OALMSGX(1, (TEXT("SET HS eMMC\r\n")));
        if (MMCCommandResponse(&MMCcmd, 0))
        {
            OALMSGX(OAL_FUNC, (TEXT("MMC: unable to set high speed mode for MMC card.\r\n")));
            goto select_card_error;
        }
        else
        {
			OALMSG(OAL_FUNC, (TEXT("SDCARD:Using High Speed mode\r\n")));
...
BOOL MMCIssueIdentify(PDISK pDisk)
{
    ...
    {
        OALMSGX(OAL_ERROR, (TEXT("MMCIssueIdentify: MMCCommandResponse Error\r\n")));
        goto command_error;
    }
    ...
    {
        OALMSGX(OAL_ERROR, (TEXT("MMCIssueIdentify: busy bit never deactivated -- no sd card detected, ocr = 0x%X\r\n"), MMCcmd.ocr));
        goto sd_det_error;
    }
...
sd_det_error:
    ...
    if ( MMC_OCR_BUSY(MMCcmd.ocr) )
    {
        OALMSGX(OAL_ERROR, (TEXT("MMCIssueIdentify: busy bit never deactivated -- probably no card, ocr = 0x%X\r\n"), MMCcmd.ocr));
        return FALSE;
    }
...
found_card:
	...
	OALMSG(OAL_INFO, (TEXT("SDCARD:Card identification complete\r\n")));
    return TRUE;
