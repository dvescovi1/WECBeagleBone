...
#define NB_KEYS 4
UCHAR vKeys[NB_KEYS] = {VK_DOWN, VK_UP, VK_RIGHT, VK_RETURN};

DWORD g_GPIOId[] = {BSP_KEY1_GPIO,BSP_KEY2_GPIO,BSP_KEY3_GPIO,BSP_KEY4_GPIO};
...
//  Called by device manager to initialize device.
{
    ...
    // make key GPIO pins inputs
    for ( i = 0; i < NB_KEYS; i++ )
    {
        GPIOSetMode(pDevice->hGpio, g_GPIOId[i], GPIO_DIR_INPUT);
    }
...
//    int index;
    ...
    while (!pDevice->bThreadExit) {
		Sleep(timeout/2);
		data = 0;
		data |= (GPIOGetBit(pDevice->hGpio,BSP_KEY1_GPIO)) ? 0x01 : 0;			  
		data |= (GPIOGetBit(pDevice->hGpio,BSP_KEY2_GPIO)) ? 0x02 : 0;			  
		data |= (GPIOGetBit(pDevice->hGpio,BSP_KEY3_GPIO)) ? 0x04 : 0;			  
		data |= (GPIOGetBit(pDevice->hGpio,BSP_KEY4_GPIO)) ? 0x08 : 0;			  
#if 0
