                      ...
                      BYTE* pSector )
{
	...
	// if returned heads is zero force it 255
    if( DiskInfo.di_heads == 0 )
	{
		DiskInfo.di_heads = 255;
		DiskInfo.di_sectors = 0x3f;
	}
...
bool ExecCommand( CSimpleString* strOp, CSimpleString* strParams )
{
    ...
    else if( _tcsicmp( strOp->GetString(), _T("WRITESEC") ) == 0 )
    {
        return WriteSector( strParams );
    }
    else if( _tcsicmp( strOp->GetString(), _T("READSEC") ) == 0 )
    {
        return ReadSector( strParams );
    }
                       ...
                       DWORD dwNumSectors )
{
    ...
    pEntry->Part_FileSystem = PART_DOS32X13;
...
void PrintCreateHelp()
{
    ...
    _tprintf( _T("Size\t\t- Specifies the size of the partition in sectors\n") );
...
void PrintZeroSectorHelp()
{
    ...
    _tprintf( _T("Sector\t-Sector to zero out.\n") );
    _tprintf( _T("\n") );
}
...
void PrintWriteSectorHelp()
{
    _tprintf( _T("\nMicrosoft Windows CE DiskPart\n\n") );
    _tprintf( _T("WRITESEC File=filename Sector=x\n") );
    _tprintf( _T("File\t-File containing the code to write to the sector(s).\n") );
    _tprintf( _T("Sector\t-Sector number to start write.\n") );
    _tprintf( _T("\n") );
}

void PrintReadSectorHelp()
{
    _tprintf( _T("\nMicrosoft Windows CE DiskPart\n\n") );
    _tprintf( _T("ReadSEC File=filename Sector=x Number=y\n") );
    _tprintf( _T("File\t-File to write containing the sector code read.\n") );
    _tprintf( _T("Sector\t-Sector number to start read.\n") );
    _tprintf( _T("Number\t-Number of sectors to read.\n") );
    _tprintf( _T("\n") );
}
...
bool CreatePartition( CSimpleString* strParams )
{
	...
	// make the size entry the number of sectors
    dwNumSectors = dwSize;
...
bool FormatPartition( CSimpleString* strParams )
{
    ...
    FormatOptions.fo.dwFatVersion = 32;
    FormatOptions.fo.dwNumFats = 2;
...
bool PrintHelp( CSimpleString* strParams )
{
    ...
    _tprintf( _T("WRITESEC      - Write a file to a sector of the disk.\n") );
    _tprintf( _T("READSEC       - Read a sector of the disk to a file.\n") );
    ...
    _tprintf( _T("FORMATPARTITION  - Format the current partition as FAT32.\n") );
    return true;
}
...
bool WriteBootSector( CSimpleString* strParams )
{
    ...
    while( strTok != NULL )
    {
        ...
        else if( _tcsnicmp( strTok, _T("sector"), 6 ) == 0 && strTok[6] == _T('=') )
        {
            ...
            if( FAILED(StringCchLength( strTok + 7, MAX_PATH, &stLength ) ) || stLength == 0 )
            {
                _tprintf( _T("Unable to read code[%d].\n"), __LINE__ );
                fResult = false;
                goto exit_writebootsector;
            }
...
bool WriteSector( CSimpleString* strParams )
{
    bool fResult = true;
    TCHAR* strTok = NULL;
    const TCHAR* Delimiters = _T(" ");
    ...
    if( g_hDisk == INVALID_HANDLE_VALUE )
    {
        _tprintf( _T("SELECT a disk before using WRITESEC.\n") );
        fResult = false;
    ...
    while( strTok != NULL )
    {
        if( _tcsnicmp( strTok, _T("file"), 4 ) == 0 && strTok[4] == _T('=') )
        {
            ...
            if( FAILED(StringCchLength( strTok + 5, MAX_PATH, &stLength ) ) )
            {
                _tprintf( _T("Unable to write code[%d].\n"), __LINE__ );
                fResult = false;
                goto exit_writesector;
            }
    ...
    if( dwByteCount != StoreInfo.dwBytesPerSector )
    {
        _tprintf( _T("The file size is not equal to a sector.\n") );
        fResult = false;
        goto exit_writesector;
    }
...
bool ReadSector( CSimpleString* strParams )
{
    bool fResult = true;
    TCHAR* strTok = NULL;
    const TCHAR* Delimiters = _T(" ");
    ...
    if( g_hDisk == INVALID_HANDLE_VALUE )
    {
        _tprintf( _T("SELECT a disk before using READSEC.\n") );
        fResult = false;
    ...
    while( strTok != NULL )
    {
        ...
        else if( _tcsnicmp( strTok, _T("number"), 6 ) == 0 && strTok[6] == _T('=') )
        {
            size_t stLength = 0;
            ...
            dwNumber = _tcstoul( strTok + 7, NULL, 0 );
        }
        else
        {
            PrintReadSectorHelp();
	...
	for (int k=0;k<dwNumber;k++,dwSector++)
	{
		IoInfo.sr_start = dwSector;
		IoInfo.sr_num_sec = 1;
		IoInfo.sr_num_sg = 1;
		...
		if (!WriteFile( hFile, (void *)pSector, dwByteCount, &dwBytesWritten, NULL )) 
		{
			_tprintf( _T("Unable to write the file: %d\n"), GetLastError() );
			fResult = false;
			break;
		}
	}
